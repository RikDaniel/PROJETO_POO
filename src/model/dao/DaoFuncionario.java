/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package model.dao;

import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.sql.SQLException;
import java.util.ArrayList;
import java.util.List;
import model.Funcionario;
/**
 *
 * @author Mayana
 */
public class DaoFuncionario {
    private Connection conexao;
    
    public DaoFuncionario() throws SQLException {
        this.conexao = CriaConexao.fazerConexao();
    }
    
    
    
    
    public void cadastroDeFuncionario( Funcionario funcionario) throws SQLException{
        String sql = "INSERT INTO FUNCIONARIO (NOME, ENDERECO, ID, TELEFONE, SALARIO, HORARIO, FUNCAO) VALUES (?,?,?,?,?,?,?,?)"; // aqui teremos a query de inserção no banco de dados
            try (PreparedStatement psmt = this.conexao.prepareStatement(sql)) {
                psmt.setString(1, funcionario.getNome());
                //psmt.setEndereco(2, funcionario.getEndereco());
                psmt.setInt(3, funcionario.getId());
                psmt.setString(4, funcionario.getTelefone());
                psmt.setDouble(5, funcionario.getSalario());
                psmt.setDouble(6, funcionario.getHorario());
                psmt.setString(7, funcionario.getFuncao());
                
                psmt.execute();
            }
        
    }   
   public void removerFuncionario(Funcionario funcionario) throws SQLException{
       String sql = "DELETE FROM FUNCIONARIO WHERE  NOME LIKE ?";
            try (PreparedStatement psmt = this.conexao.prepareStatement(sql)) {
                psmt.setString(1, funcionario.getNome());
                psmt.execute();
            }
       
   }
   public void atualizarFuncionario(Funcionario funcionario, int id) throws SQLException{
       String sql = " UPDATE FUNCIONARIO SET NOME = ?, ENDERECO = ?, ID = ?, TELEFONE = ?, SALARIO = ?, HORARIO = ?, FUNCAO = ? WHERE COD LILE ?";
            try (PreparedStatement psmt = this.conexao.prepareStatement(sql)) {
                psmt.setString(1, funcionario.getNome());
                //psmt.setEndereco(2, funcionario.getEndereco());
                psmt.setInt(3, funcionario.getId());
                psmt.setString(4, funcionario.getTelefone());
                psmt.setDouble(5, funcionario.getSalario());
                psmt.setDouble(6, funcionario.getHorario());
                psmt.setString(7, funcionario.getFuncao());
               
               psmt.setInt(3, id);
               
               psmt.execute();
            } 
   }
   
   public List<Funcionario> selecionarFuncionario(String nome) throws SQLException{
       List<Funcionario> listaDao = new ArrayList<>();
       
       String sql = "SELECT * FROM FUNCIONARIO WHERE NOME LIKE ?";
            try (PreparedStatement psmt = this.conexao.prepareStatement(sql)) {
                psmt.setString(1, nome);
           try (ResultSet rs = psmt.executeQuery()) {
               while(rs.next()){
                   Funcionario listaFuncionario = new Funcionario();
                   
                   listaFuncionario.setNome(rs.getString("NOME"));
                   //listaFuncionario.setEndereco(rs.getString("Endereco"));
                   listaFuncionario.setId(rs.getInt("ID"));
                   listaFuncionario.setTelefone(rs.getString("TELEFONE"));
                   listaFuncionario.setSalario(rs.getDouble("SALARIO"));
                   listaFuncionario.setHorario(rs.getDouble("HORARIO"));
                   listaFuncionario.setFuncao(rs.getString("FUNCAO"));
                   listaDao.add(listaFuncionario);
               }
           }
            }
       
        
        
       return listaDao;
   }
    
}
